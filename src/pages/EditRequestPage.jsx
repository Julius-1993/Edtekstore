...
export default function EditRequestPage() {
  ...
  useEffect(() => {
    api.get(`/requests/${id}`).then(res => {
      ...
      if (r.status !== 'draft') { toast.error('Only draft requests can be edited'); navigate(`/requests/${id}`); return }
      setForm({
        toOrganization: r.toOrganization, toDepartment: r.toDepartment, priority: r.priority,
        requestNotes: r.requestNotes||'', contactPerson: r.contactPerson||'', contactPhone: r.contactPhone||'',
        contactEmail: r.contactEmail||'', deliveryAddress: r.deliveryAddress||'',
        expectedDeliveryDate: r.expectedDeliveryDate ? r.expectedDeliveryDate.split('T')[0] : ''
      })
      setItems(r.items.map(i => ({
        stockId: i.stock?._id || i.stock, serialNumber: i.serialNumber, name: i.name,
        screenSize: i.screenSize, specification: i.specification, unit: i.unit,
        available: i.stock?.quantityRemaining ?? 9999, category: i.category, quantity: i.quantityRequested
      })))
  ...
  const handleSaveDraft = async () => {
    ...
    setSavingDraft(true)
    try {
      await api.put(`/requests/${id}/draft`, buildPayload())
      toast.success('Draft updated!')
      navigate(`/requests/${id}`)
    } catch (err) { toast.error(err.response?.data?.message || 'Failed to update draft') }
  ...
  const ItemsForm = () => (
    <div className="card-pad">
      <h3 className="font-bold text-sm mb-4 pb-3 border-b border-slate-100" style={{ color:'#020c1b' }}>
        Items <span className="ml-1 text-xs font-bold px-2 py-0.5 rounded-full" style={{ background:'rgba(26,86,219,0.1)', color:'#1a56db' }}>{items.length}</span>
      </h3>
              ...
              <button key={s._id} type="button" onClick={() => addItem(s)}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs mt-0.5" style={{ color:'#64748b' }}><span className="font-mono">{s.serialNumber}</span> · {categoryLabel(s.category)}</p>
                  </div>
                  <div className="text-right ml-3 flex-shrink-0"><p className="text-sm font-bold text-green-600">{s.quantityRemaining}</p><p className="text-xs text-slate-400">available</p></div>
                </div>
              </button>
            ))}
          </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-10 border-2 border-dashed border-slate-200 rounded-xl"><p className="text-sm text-slate-400">Search and add items above</p></div>
      ) : (
        <div className="space-y-2">
          {items.map((item, idx) => (
            <div key={item.stockId} className="flex items-center gap-3 p-3 rounded-xl" style={{ background:'#f8fafc' }}>
              <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 text-xs font-bold" style={{ background:'rgba(26,86,219,0.1)', color:'#1a56db' }}>{idx + 1}</div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button type="button" onClick={() => setItems(p => p.filter(i => i.stockId !== item.stockId))} className="btn btn-ghost btn-sm btn-circle text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="fade-up max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        <Link to={`/requests/${id}`} className="btn btn-ghost btn-sm gap-2 text-sm"><ArrowLeft className="w-4 h-4" /> Back</Link>
        <div><h1 className="page-title">Edit Draft Request</h1><p className="page-subtitle">Make changes before submission</p></div>
      </div>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <button type="button" onClick={() => { if(validate()) setShowPreview(true) }} className="btn btn-outline gap-2"><Eye className="w-4 h-4" /> Preview</button>
          ...
          <Link to={`/requests/${id}`} className="btn btn-ghost">Cancel</Link>
        </div>
      </form>
    </div>
  )
